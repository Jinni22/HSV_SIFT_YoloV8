...
image_path = "video_data/videoYellowFish/yellowFish_Image.png"
image = cv2.imread(image_path)

# Convert the image to HSV color space
hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)

# Define the yellow color range in HSV (optimized parameters)
lower_yellow = np.array([24, 92, 175])
upper_yellow = np.array([85, 255, 255])

# Create a mask to isolate the yellow regions
mask = cv2.inRange(hsv, lower_yellow, upper_yellow)
segmented_fish = cv2.bitwise_and(image, image, mask=mask)

# Convert the segmented fish image to grayscale for SIFT feature extraction
gray_fish = cv2.cvtColor(segmented_fish, cv2.COLOR_BGR2GRAY)
sift = cv2.SIFT_create()
keypoints1, descriptors1 = sift.detectAndCompute(gray_fish, None)

# Display the SIFT keypoints on the segmented fish
keypoint_image = cv2.drawKeypoints(
    gray_fish, keypoints1, None, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS
)
plt.imshow(keypoint_image, cmap='gray')
plt.title('SIFT Keypoints on Segmented Fish')
plt.show()
print(f"Number of extracted keypoints: {len(keypoints1)}")

# Load the video and initialize the FLANN matcher
video_path = "video_data/videoYellowFish/yellowFish.mp4"
